<template>
  <div class="relative flex gap-4 pt-5 px-5 -mx-5 -mt-5 overflow-hidden bg-[radial-gradient(ellipse_at_top,_rgba(28,176,246,0.18),_transparent_62%)]">
    <div class="min-w-0 flex-1 space-y-0.5">
      <div class="flex items-center gap-2 min-w-0">
        <h2 class="text-xl font-bold text-white truncate">{{ displayName }}</h2>
        <slot name="badge" />
      </div>
      <p class="text-slate-400 text-sm font-medium truncate">@{{ username }}</p>
