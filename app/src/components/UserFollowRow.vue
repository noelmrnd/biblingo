<template>
  <div
    @click="$emit('open')"
    :class="user.is_self ? 'cursor-default' : 'cursor-pointer hover:bg-slate-800/80 hover:border-slate-700'"
    class="w-full text-left p-3 rounded-2xl border border-slate-800 bg-slate-950/60 transition-all flex items-center gap-3"
  >
    ...
    <span v-if="user.is_following" class="text-sm text-slate-500 font-medium flex-none">Siguiendo</span>
    <AppButton
